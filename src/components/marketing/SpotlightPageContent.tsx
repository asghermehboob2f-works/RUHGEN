  ...
  return (
    <>
      ...
      <section className="py-20 border-b" style={{ borderColor: "var(--border-subtle)" }}>
        <div className={SITE_CONTAINER}>
          <header className="mb-14 max-w-3xl text-center mx-auto">
            <div className="inline-flex items-center gap-2 rounded-full border border-[#7B61FF]/30 bg-gradient-to-r from-[#7B61FF]/10 to-transparent px-4 py-1.5 text-xs font-bold uppercase tracking-[0.2em] mb-4 text-[#7B61FF] shadow-[0_0_15px_rgba(123,97,255,0.05)]">
              <Layers className="h-4 w-4 text-[#7B61FF]" />
              Platform Features
            </div>
            <h2 className="font-display text-3xl font-extrabold tracking-tight sm:text-4xl text-white">
              Engineered for absolute creative control
            </h2>
            <p className="mt-4 text-sm max-w-xl mx-auto leading-relaxed text-[var(--text-muted)]">
              Surgical editing toolkits, high-fidelity pipelines, and latency-optimized orchestrations built into a seamless workspace.
            </p>
          </header>

          <div className="grid gap-6 md:grid-cols-3">
            {content.spotlightFeatures?.map((feat, i) => {
              const Icon = [Zap, Wand2, Activity, Cpu, Sparkles, Layers][i % 6];
              return (
              <motion.div
                key={feat.id}
                ...
                viewport={{ once: true, margin: "-40px" }}
                transition={{ duration: 0.5, delay: i * 0.1 }}
                className="group relative overflow-hidden rounded-[20px] border p-5 sm:p-6 flex flex-col justify-between transition-all duration-300 hover:border-white/10 hover:-translate-y-1"
                style={{
                  borderColor: "rgba(255,255,255,0.04)",
                  background: "linear-gradient(145deg, rgba(20,20,25,0.8) 0%, rgba(10,10,12,0.95) 100%)",
                  boxShadow: "0 10px 30px -10px rgba(0,0,0,0.5), inset 0 1px 0 rgba(255,255,255,0.02)",
                }}
              >
                {/* Subtle Grain & Inner Glow */}
                <div className="absolute inset-0 bg-[url('/noise.png')] opacity-20 mix-blend-overlay pointer-events-none" />
                <div
                  className="absolute -inset-px rounded-[20px] opacity-0 transition-opacity duration-500 group-hover:opacity-100 pointer-events-none"
                  style={{
                    background: `radial-gradient(400px circle at 50% -30px, ${feat.glowColor}10, transparent 60%)`,
                  }}
                />
                ...
                <div className="relative z-10">
                  <div className="flex items-center justify-between mb-4">
                    {/* Icon Frame */}
                    <div
                      className="flex h-10 w-10 items-center justify-center rounded-[10px] border transition-transform duration-500 group-hover:scale-105"
                      style={{ 
                        borderColor: `${feat.glowColor}25`, 
                        background: `linear-gradient(135deg, ${feat.glowColor}15 0%, transparent 100%)`,
                        boxShadow: `inset 0 1px 0 ${feat.glowColor}10`
                      }}
                    >
                      <Icon className="h-4.5 w-4.5 group-hover:rotate-6 transition-transform duration-500" style={{ color: feat.glowColor }} />
                    </div>
                  </div>

                  <h3 className="font-display text-base font-bold tracking-tight text-white group-hover:text-white/90 transition-colors duration-300">
                    {feat.title}
                  </h3>
                  ...
                  <div className="mt-2 flex items-center gap-1.5">
                    ...
                    <span className="font-mono text-[8.5px] text-white/30 tracking-widest uppercase font-semibold">Latency &lt; 12ms</span>
                  </div>

                  <p className="mt-3 text-[13px] leading-relaxed text-[var(--text-subtle)] font-light">
                    {feat.description}
                  </p>
                </div>

                {/* Bottom Bezel Deco */}
                <div className="relative z-10 mt-6 pt-3 border-t border-white/[0.03] flex items-center justify-between text-white/20 text-[9px] font-mono tracking-wider">
                  <span>SYSTEM_NODE // active</span>
                  <span className="group-hover:text-white/40 transition-colors duration-300">CTRL // 0{i+1}</span>
                </div>
              </motion.div>
              );
            })}
