...
export function LuxuryStudioLayout({
  ...
  return (
    <LuxuryStudioChromeProvider collapsed={collapsed} onToggleCollapsed={() => setCollapsed((c) => !c)}>
      <div
        className="luxury-studio-root font-studio-sans flex min-h-0 flex-1 flex-col overflow-hidden px-3 pb-[max(0.5rem,env(safe-area-inset-bottom,0px))] pt-3 max-sm:h-[calc(100dvh-env(safe-area-inset-top,0px)-3.5rem)] sm:px-4 sm:pt-3 lg:h-[calc(100dvh-env(safe-area-inset-top,0px))] lg:max-h-[calc(100dvh-env(safe-area-inset-top,0px))] lg:px-5 lg:pb-4 lg:pt-4 bg-[#09090b]"
        data-studio-mode={mode}
        data-mobile-pane={mobilePane}
        onTouchStart={swipe.onTouchStart}
        onTouchEnd={swipe.onTouchEnd}
      >
        <div className="pointer-events-none absolute inset-0 z-[1] opacity-[0.025] app-grain" aria-hidden />

        <header className="relative z-10 mb-2 shrink-0 lg:mb-3">
          <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#121215] p-2 shadow-sm sm:gap-3 sm:p-2.5 lg:pl-3.5">
            <span
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border border-white/15 bg-zinc-800 text-zinc-100 sm:h-10 sm:w-10 shadow-sm"
            >
              <Sparkles className="h-4 w-4 sm:h-5 sm:w-5 text-zinc-200" strokeWidth={1.75} />
            </span>
            ...
            <div className="min-w-0 flex-1 overflow-hidden">
              ...
              <div className="mt-0.5 sm:mt-1 flex min-w-0 items-baseline gap-x-2.5">
                <h1 className="truncate font-display text-[13px] sm:text-base font-bold leading-tight tracking-tight text-white">
                  {title}
                </h1>
                <p className="hidden min-w-0 max-w-[min(100%,28rem)] truncate text-[11px] font-normal leading-snug text-zinc-400 lg:block">
                  {subtitle}
                </p>
              </div>
            </div>
            ...
            <nav
              className="ml-auto flex shrink-0 gap-1 rounded-lg border border-white/10 bg-zinc-950 p-1"
        ...
        <div className="relative z-10 flex min-h-0 flex-1 flex-col overflow-hidden">
          <div
            className="mb-2 flex shrink-0 gap-1.5 rounded-xl border border-white/10 p-1 bg-zinc-950 lg:hidden"
            role="tablist"
          ...
          >
            {(["controls", "output"] as const).map((pane) => (
              <button
                ...
                style={{
                  background: mobilePane === pane ? "var(--soft-black)" : "transparent",
                  color: mobilePane === pane ? "#ffffff" : "var(--text-muted)",
                  border:
                    mobilePane === pane ? `1px solid rgba(255, 255, 255, 0.15)` : "1px solid transparent",
                }}
              >
                {pane === "output" ? (
          ...
          >
            <motion.aside
              ...
              className={`flex min-h-0 min-w-0 flex-col h-full w-full overflow-hidden ${
                mobilePane === "controls" ? "max-lg:flex max-lg:flex-1" : "max-lg:hidden"
              } ${collapsed ? "lg:hidden" : "lg:flex"}`}
              aria-label="Generation controls"
            >
              <div className="flex min-h-0 flex-1 flex-col rounded-xl border border-white/10 bg-[#121215] shadow-sm h-full max-h-full overflow-hidden">
                {leftPanel}
              </div>
            </motion.aside>
